...
// S_IEXEC is an obsolete synonym btw...
#ifndef S_IEXEC
#define S_IEXEC S_IXUSR
#endif

// private Haiku kernel function
...
CreateExecutionEnvironment(int *_argc,
	...
	// set the files open to a larger number than the default
	_kset_fd_limit_(512);
...
#ifdef HAIKU
    libjvm = dlopen(jvmpath, RTLD_HAIKU);
#else
...
#ifdef HAIKU
    dladdr(dlsym(dlopen(JVM_DLL, RTLD_HAIKU), "JNI_CreateJavaVM"), &dlinfo);
#else
	       ...
	       char *jrepath, char *_jvmpath)
{
    ...
    new_runpath = MemAlloc( ((runpath!=NULL)?strlen(runpath):0) + 
			   ...
			   strlen(jvmpath) + 52);
#ifdef __HAIKU__
    newpath = new_runpath + strlen("LIBRARY_PATH=");
#else
...
#ifdef __HAIKU__
    sprintf(new_runpath, "LIBRARY_PATH="
#else
	...
	/*
	 * On Haiku libdl.so honors the new LIBRARY_PATH setting,
