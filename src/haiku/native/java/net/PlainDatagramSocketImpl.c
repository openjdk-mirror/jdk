...
Java_java_net_PlainDatagramSocketImpl_peek(JNIEnv *env, jobject this,
    ...
    len = SOCKADDR_LEN;
    debugger("TODO: IMPLEMENT MSG_PEEK");
...
#endif /* AF_INET6 */
	{
		...
		CHECK_NULL(ia_addressID);
            ...
            if (JVM_SetSockOpt(fd, IPPROTO_IP, IP_MULTICAST_IF,
                ...
                // R5 = ENOPROTOOPT => silently succeed
    ...
    if (NET_SetSockOpt(fd, level, optname, (const void *)&optval, optlen) < 0) {
    	if (errno == ENOPROTOOPT) {
	        switch (opt) {
	            case java_net_SocketOptions_SO_SNDBUF:
	            case java_net_SocketOptions_SO_RCVBUF:
	            	// SNDBUF/RCVBUF are hints, if ENOPROTOOPT, silently succeed
	            	return;
	            case java_net_SocketOptions_SO_BROADCAST:
	            	break;
	            case java_net_SocketOptions_IP_MULTICAST_LOOP:
...
jobject getMulticastInterface(JNIEnv *env, jobject this, int fd, jint opt) {
	...
	if (isOldKernel) {
        ...
        if (JVM_GetSockOpt(fd, IPPROTO_IP, IP_MULTICAST_IF,
            ...
            inP->s_addr = ntohl(INADDR_BROADCAST);
...
#endif
	fprintf(stderr, "NET_GET_SOCK_OPT\n");
	        if (NET_GetSockOpt(fd, IPPROTO_IP, IP_MULTICAST_IF, (char *)inP, &len) < 0) {
	        	if (errno != ENOPROTOOPT) {
