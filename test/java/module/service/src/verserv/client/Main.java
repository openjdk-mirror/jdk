...
import verserv.service.FooService;
...
public class Main {
    public static void realMain(String[] args) throws Throwable {
        ...
        check(clazz.getName().equals("verserv.service.FooServiceDefaultProvider"));
        checkVersion(clazz, "1.0");

        // Check that the next service is the non-default provider, and that
        // it is the higher of the two versions that are compatible with with
        // the constraint specified by this client.
        fs = FooService.getNextProvider();
        clazz = fs.getClass();
        check(clazz.getName().equals("verserv.provider.FooService2Provider"));
        checkVersion(clazz, "1.5");
    ...
    static void checkVersion(Class clazz, String expectedVersion) throws Throwable {
