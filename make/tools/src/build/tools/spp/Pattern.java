    ...
    implements java.io.Serializable
{
    ...
    private void normalize() {
        ...
        for(int i=0; i<patternLength; ) {
            int c = codePointAt(normalizedPattern, i);
            StringBuilder sequenceBuffer;
            if ((CharactergetType(c) == Character.NON_SPACING_MARK)
                && (lastCodePoint != -1)) {
                sequenceBuffer = new StringBuilder();
                sequenceBuffer.appendCodePoint(lastCodePoint);
                sequenceBuffer.appendCodePoint(c);
                while(CharactergetType(c) == Character.NON_SPACING_MARK) {
                    i += charCount(c);
                    if (i >= patternLength)
                        break;
                    c = codePointAt(normalizedPattern, i);
                    sequenceBuffer.appendCodePoint(c);
                }
                String ea = produceEquivalentAlternation(
                                               sequenceBuffer.toString());
                newPattern.setLength(newPattern.length()-charCount(lastCodePoint));
    ...
    public static int charCount(int codePoint) {
        return codePoint >= AbstractStringBuilder.MIN_SUPPLEMENTARY_CODE_POINT? 2 : 1;
    }
     ...
     * @since   1.5
     */
    public static int CharactergetType(int codePoint) {
     ...
     * @since      1.5
     */
    public static int codePointAt(CharSequence string, int index) {
        if ((index < 0) || (index >= string.length())) {
            throw new StringIndexOutOfBoundsException(index);
        }
        return codePointAtImpl(string, index, string.length());
    }

    static int codePointAtImpl(CharSequence string, int index, int limit) {
        char c1 = string.charAt(index++);
        if (AbstractStringBuilder.isHighSurrogate(c1)) {
            if (index < limit) {
                char c2 = string.charAt(index);
                if (AbstractStringBuilder.isLowSurrogate(c2)) {
                    return AbstractStringBuilder.toCodePoint(c1, c2);
                }
            }
        }
        return c1;
    ...
    private int normalizeCharClass(StringBuilder newPattern, int i) {
        ...
        while(true) {
            ...
            if (c == ']' && lastCodePoint != '\\') {
                ...
                break;
            } else if (CharactergetType(c) == Character.NON_SPACING_MARK) {
                sequenceBuffer = new StringBuilder();
                sequenceBuffer.appendCodePoint(lastCodePoint);
                while(CharactergetType(c) == Character.NON_SPACING_MARK) {
                    sequenceBuffer.appendCodePoint(c);
                ...
                charClass.setLength(charClass.length()-charCount(lastCodePoint));
                if (eq == null)
    ...
    private String[] producePermutations(String input) {
        ...
        if (input.length() == countChars(input, 0, 2)) {
            int c0 = codePointAt(input, 0);
            int c1 = codePointAt(input, charCount(c0));
...
loop:   for(int x=0, offset=0; x<nCodePoints; x++, offset+=len) {
    ...
    private int getClass(int c) {
        throw new UnsupportedOperationException("return sun.text.Normalizer.getCombiningClass(c);");
    }
    ...
    private void compile() {
        ...
        for (int x = 0; x < patternLength; x += charCount(c)) {
            c = codePointAt(normalizedPattern, x);
    ...
    private static final boolean isSupplementary(int ch) {
        return ch >= AbstractStringBuilder.MIN_SUPPLEMENTARY_CODE_POINT || isSurrogate(ch);
    }
    ...
    private int escape(boolean inclass, boolean create) {
        ...
        case 'k':
            ...
            if (create) {
                if (has(CASE_INSENSITIVE))
                    root = new CIBackRef(((Integer)namedGroups().get(name)).intValue(), has(UNICODE_CASE));
                else
                    root = new BackRef(((Integer)namedGroups().get(name)).intValue());
            }
            return -1;
        case 'l':
        case 'm':
    ...
    public static boolean isSupplementaryCodePoint(int codePoint) {
        return codePoint >= AbstractStringBuilder.MIN_SUPPLEMENTARY_CODE_POINT
            && codePoint <= AbstractStringBuilder.MAX_CODE_POINT;
    }
    ...
    public String newString(int[] codePoints, int offset, int count) {
        if (offset < 0) {
        ...
        for (int i = offset; i < offset + count; i++) {
            int c = codePoints[i];
            if (c >= AbstractStringBuilder.MIN_CODE_POINT &&
                c <  AbstractStringBuilder.MIN_SUPPLEMENTARY_CODE_POINT)
                n += 1;
            else if (isSupplementaryCodePoint(c))
                n += 2;
        ...
        for (int i = offset, j = 0; i < offset + count; i++) {
            int c = codePoints[i];
            if (c < AbstractStringBuilder.MIN_SUPPLEMENTARY_CODE_POINT) {
                v[j++] = (char) c;
            } else {
                AbstractStringBuilder.toSurrogates(c, v, j);
                j += 2;
            }
        }
    ...
    private CharProperty unicodeBlockPropertyFor(String name) {
    	throw new UnsupportedOperationException("Character.UnicodeBlock.forName");
...
//                    return block == Character.UnicodeBlock.of(ch);}};
    ...
    public static char[] toChars(int codePoint) {
        if (codePoint < 0 || codePoint > AbstractStringBuilder.MAX_CODE_POINT) {
            throw new IllegalArgumentException();
        }
        if (codePoint < AbstractStringBuilder.MIN_SUPPLEMENTARY_CODE_POINT) {
                return new char[] { (char) codePoint };
        }
        char[] result = new char[2];
        AbstractStringBuilder.toSurrogates(codePoint, result, 0);
        return result;
    }
    ...
    private int u() {
        int n = uxxxx();
        if (AbstractStringBuilder.isHighSurrogate((char)n)) {
            int cur = cursor();
            if (read() == '\\' && read() == 'u') {
                int n2 = uxxxx();
                if (AbstractStringBuilder.isLowSurrogate((char)n2))
                    return AbstractStringBuilder.toCodePoint((char)n, (char)n2);
            }
            setcursor(cur);
        }
        return n;
    ...
    private static final boolean isSurrogate(int c) {
        return c >= AbstractStringBuilder.MIN_HIGH_SURROGATE && c <= AbstractStringBuilder.MAX_LOW_SURROGATE;
    }
    ...
    private Node newSlice(int[] buf, int count, boolean hasSupplementary) {
        ...
        if (has(CASE_INSENSITIVE)) {
            ...
            if (hasSupplementary) {
            	return new SliceIS(tmp);
            } else {
            	return new SliceI(tmp);
            }
        }
        for (int i = 0; i < count; i++) {
            tmp[i] = buf[i];
    ...
    private static abstract class CharProperty extends Node {
        ...
        boolean match(Matcher matcher, int i, CharSequence seq) {
            if (i < matcher.to) {
                int ch = codePointAt(seq, i);
                return isSatisfiedBy(ch)
                    && next.match(matcher, i+charCount(ch), seq);
            } else {
                matcher.hitEnd = true;
                return false;
            }
    ...
    static final class Category extends CharProperty {
        ...
        boolean isSatisfiedBy(int ch) {
            return (typeMask & (1 << CharactergetType(ch))) != 0;
        }
    }
    ...
    static class CIBackRef extends Node {
        ...
        boolean match(Matcher matcher, int i, CharSequence seq) {
            ...
            int x = i;
            for (int index=0; index<groupSize; index++) {
                int c1 = codePointAt(seq, x);
                int c2 = codePointAt(seq, j);
                if (c1 != c2) {
                    if (doUnicodeCase) {
        ...
        int check(Matcher matcher, int i, CharSequence seq) {
            ...
            if (i > startIndex) {
                ...
                left = (ch == '_' || Character.isLetterOrDigit(ch) ||
                    ((CharactergetType(ch) == Character.NON_SPACING_MARK)
                     && hasBaseCharacter(matcher, i-1, seq)));
            }
            boolean right = false;
            if (i < endIndex) {
                ch = codePointAt(seq, i);
                                            ...
                                            CharSequence seq)
    {
        ...
        for (int x=i; x >= start; x--) {
            int ch = codePointAt(seq, x);
            if (Character.isLetterOrDigit(ch))
                return true;
            if (CharactergetType(ch) == Character.NON_SPACING_MARK)
                continue;
            return false;
        }
        return false;
    ...
    static final class BnMS extends BnM {
        ...
        BnMS(int[] src, int[] lastOcc, int[] optoSft, Node next) {
            ...
            for (int x = 0; x < buffer.length; x++) {
                lengthInChars += charCount(buffer[x]);
            }
        }
        boolean match(Matcher matcher, int i, CharSequence seq) {
            int[] src = buffer;
...
///////////////////////////////////////////////////////////////////////////////
    ...
    private static class CharPropertyNames {
        ...
        static CharProperty charPropertyFor(String name) {
            CharPropertyFactory m = (CharPropertyFactory)map.get(name);
